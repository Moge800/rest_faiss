...
- **sentence-transformers**: 多言語対応文章埋め込みモデル（intfloat/multilingual-e5-large使用）
- **pandas**: データ処理
- **uvicorn**: ASGI サーバー
...
2. **高精度検索**: intfloat/multilingual-e5-largeモデルによる日本語に最適化された検索
3. **多言語対応**: sentence-transformersの多言語モデルを使用
4. **固有名詞正規化**: 略称や表記ゆれに対応
5. **スケーラブル**: 大規模データセットに対応可能
6. **型安全**: Pydanticによる型検証
7. **動的カラム対応**: CSVのカラム構造が変わっても自動対応
8. **モデル最適化**: シングルトンパターンによる効率的なメモリ使用
